...
const Skills = () => {
  ...
  return <section id="skills" className="py-24 bg-gradient-to-b from-cyan-50 via-blue-50 to-purple-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
          {skillCategories.map((category, index) => <Card key={category.title} className={`p-8 bg-gradient-to-br ${category.bgGradient} border-0 shadow-2xl hover:shadow-2xl transition-all duration-500 group hover:scale-110`} style={{
          animationDelay: `${index * 100}ms`
              ...
              <div className="space-y-4">
                {category.skills.map(skill => <div key={skill} className="flex items-center group/skill hover:scale-105 transition-transform">
        ...
        <div className="text-center">
          <h3 className="text-4xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent mb-12">Technical Superpowers </h3>
          ...
          }].map((skill, index) => <span key={skill.name} className={`px-8 py-4 bg-gradient-to-r ${skill.gradient} text-white rounded-2xl hover:scale-110 transition-all duration-300 cursor-default shadow-lg hover:shadow-xl font-bold text-lg`} style={{
